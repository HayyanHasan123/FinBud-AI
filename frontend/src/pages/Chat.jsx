...
export default function Chat() {
  ...
  // ── EMERGENCY CARD-SELECTION MODAL ───────────────────────
  // Shown ahead of the password modal when the account has 2+ cards, so
  // locking one doesn't silently lock all of them. Styled to match the
  // existing pw-modal so it doesn't feel like a bolted-on component.
  function CardSelectModal() {
    const cards = pendingInfo?.cards || []
    return (
      <div className="pw-modal emergency">
        <button className="pw-close" aria-label="Close dialog" onClick={() => setModal(null)}>×</button>
        <h3>Which card would you like to lock?</h3>
        <p style={{ fontSize: 14, color: '#666', marginBottom: 16 }}>
          You have {cards.length} cards on file.
        </p>
        <div className="pw-summary" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {cards.map((c, i) => (
            <button
              key={c.card_id ?? i}
              type="button"
        ...
        <button type="button" className="pw-btn-secondary" style={{ marginTop: 12 }} onClick={() => setModal(null)}>CANCEL</button>
      </div>
    )
  }
      ...
      <div className="main-layout">
        ...
        <aside className="sidebar-right">
          ...
          <div className={`sidebar-card emergency-card ${!hasCard ? 'no-card' : ''}`} role="button" tabIndex={0}
            ...
            <div className="sidebar-card-desc">{hasCard ? 'Lock a card now' : 'No card registered'}</div>
          </div>
        </aside>
      </div>
      ...
      {modal && modal !== 'processing' && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div onClick={e => e.stopPropagation()}>
            ...
            {modal === 'emergencyCardSelect' && <CardSelectModal />}
