...
export default function InvestingPanel() {
  ...
  return (
    <div style={S.outer}>
      ...
      */}
      {/* Inline styles can't respond to viewport width on their own, so the
          responsive column count for the type grid below lives here — 3
          compact cards per row on phones instead of the cards stacking
      ...
      <style>{`
        @media (max-width: 640px) {
          ...
          .investing-type-card-icon {
            font-size: 16px !important;
          }
          .investing-type-card-label {
      ...
      <div style={S.card}>
        {!selectedType ? (
          <>
            ...
            {!loaded ? (
              <div className="investing-type-grid" style={S.grid}>
              ...
              <div className="investing-type-grid" style={S.grid}>
                {types.map((type) => {
                  ...
                  return (
                    <button key={type.value} type="button" className="investing-type-card" onClick={() => openGuide(type)} style={S.typeCard}>
                      <i className={`fa-solid ${type.icon} investing-type-card-icon`} style={S.typeCardIcon} aria-hidden="true" />
                      <span className="investing-type-card-label" style={S.typeCardLabel}>{shortLabel}</span>
                    </button>
                  )
                })}
              </div>
