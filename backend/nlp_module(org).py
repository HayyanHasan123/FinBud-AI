...
# backwards over.
FLOW_SLOT_ORDER = {
    'transfer_money': ['recipient', 'account_number', 'amount'],
...
INTENT_PATTERNS = {
    ...
    'pay_bill': [
        # Original Roman Urdu words
        r'\b(bill|bijli|bijlee|bjili|bijali|bijly|pani|paani|gas|sui\s*gas|suigas|sui\s*gais)\b',
...
RESPONSES = {
    ...
    'transfer_ask_amount': {
        'en': "💰 How much would you like to send to {recipient}?",
        'ur': "💰 \u200Fآپ \u2066{recipient}\u2069 کو کتنی رقم بھیجنا چاہتے ہیں؟",
        'ru': "💰 Aap {recipient} ko kitni raqam bhejna chahte hain?"
    },
...
# ── Recipient-name extraction guards ────────────────────────────────────────
# Every word/phrase below must NEVER be captured as part of a `recipient`
# value. Kept as explicit, categorized sets (rather than one flat grab-bag
# list) so the stoplist stays easy to audit and extend as new garbage tokens
# are discovered in production logs, and so English and Roman Urdu coverage
# can be reasoned about independently.

# Command / intent verbs - English. Includes modal/desire verbs ("want",
# "would", "need") that earlier caused phrases like "I want to send" to leak
# "I Want" through as a fake recipient name.
RECIPIENT_STOP_COMMAND_VERBS_EN = {
    ...
    'like', 'likes', 'please', 'kindly', 'help', 'can', 'could', 'should',
    'will', 'shall', 'have', 'has',
}

# Command / intent verbs - Roman Urdu. Covers imperative, infinitive, and
# conjugated forms of "bhejna" (to send), "karna" (to do), and "chahna"
# (to want), including common alternate spellings, so e.g. "karne" (the
# gerund/infinitive form that was previously missing) no longer slips
# through as a name.
RECIPIENT_STOP_COMMAND_VERBS_RU = {
    'bhejo', 'bhej', 'bhejdo', 'bhejna', 'bhejiye', 'bhejein', 'bhejenge',
    'bhejta', 'bhejti', 'bhejte', 'bhejne', 'bhejwao', 'bhijwana', 'bhijwao', 'bejo',
    'bhej', 'karo', 'karna', 'karne', 'karni', 'karein', 'karenge', 'kare',
    'kijiye', 'krna', 'krni', 'krne', 'krdo', 'kardo', 'kardein', 'kro',
    'kr', 'chahta', 'chahti', 'chahte', 'chahiye', 'chahye', 'mangta',
    'mangti', 'mangte', 'de', 'dena', 'deta', 'deti', 'dete', 'dijiye',
    'den', 'dein',
}

# Prepositions - English + Roman Urdu.
RECIPIENT_STOP_PREPOSITIONS = {
    'to', 'for', 'from', 'into', 'towards', 'ko', 'ka', 'ki', 'ke', 'mein',
    'se', 'par', 'pe',
}

# Pronouns, articles, copulas, and other grammar particles that a name
# extractor could otherwise mistake for a leftover proper noun. Roman Urdu
# has no standardized spelling, so several common variant spellings of the
# same pronoun are listed explicitly (e.g. "mujhe"/"mujhay"/"mujhy" all mean
# "to me").
RECIPIENT_STOP_PRONOUNS_PARTICLES = {
    'i', 'me', 'my', 'mine', 'we', 'us', 'our', 'you', 'your', 'he', 'she',
    'it', 'they', 'them', 'him', 'her', 'a', 'an', 'the', 'is', 'am', 'are',
    'was', 'were',
    # "to me" - mujhe and common misspellings
    'mujhe', 'mujhay', 'mujhy',
    # "to us" - hume/humein and misspellings
    'humein', 'hume', 'humay', 'humy',
    # "to you" - tumhe/aapko and misspellings
    'tumhe', 'tumhay', 'tumhein', 'apko', 'aapko',
    # "to him/her/it" - usko/isko and misspellings
    'usko', 'unko', 'ise', 'isko', 'ussay', 'usay', 'unhay', 'unhe',
    'isay',
    'aap', 'hai', 'hain', 'hy', 'hen', 'ha', 'hoon', 'hun', 'ho', 'tha',
    'thi', 'do', 'ye', 'yeh', 'woh', 'wo', 'main', 'hum', 'tum', 'aur',
    'ya', 'bhi', 'hi', 'toh', 'that', 'this', 'and', 'or',
}

# Financial/currency stopwords - these must never be mistaken for a name
# regardless of language.
RECIPIENT_STOP_CURRENCY_FINANCIAL = {
    'rs', 'pkr', 'rupees', 'rupee', 'rupay', 'rupaye', 'rupaya', 'rupye',
    'money', 'paisa', 'paise', 'paisay', 'paisy', 'rakam', 'raqam',
    'amount', 'cash', 'fund', 'funds', 'payment',
}

# Multi-word phrases that need to be stripped as a unit before per-word
# filtering runs, because splitting them into individual words first (or
# relying on individual-word filtering alone) can leave a stray connective
# behind, e.g. "want to" -> filtering "want" and "to" separately still
# works, but phrases like "bhej do" and "chahta hoon" read more reliably
# as a single unit.
RECIPIENT_STOP_MULTIWORD_PHRASES = [
    'bhej do', 'bhej dena', 'kar do', 'kar dena', 'karna hai', 'karni hai',
    'karne hain', 'chahta hoon', 'chahti hoon', 'chahte hain',
    'want to', 'wants to', 'would like to', 'need to', 'have to', 'has to',
    'i want', 'i need', 'i would like',
]

# The full single-word lookup set used by the extractor - every category
# above, unioned into one set.
RECIPIENT_STOPWORDS = (
    RECIPIENT_STOP_COMMAND_VERBS_EN
    | RECIPIENT_STOP_COMMAND_VERBS_RU
    | RECIPIENT_STOP_PREPOSITIONS
...
# ── Roman Urdu verb root+suffix matcher ─────────────────────────────────────
# Roman Urdu has no standardized spelling, so no fixed word list will ever
# cover every conjugation/typo a user types (e.g. "bhejnei" vs "bhejne" vs
# "bhejnay" - all the same word, three different spellings). Rather than
# playing whack-a-mole adding one spelling at a time, this checks whether a
# word is [known verb root] + [known suffix], which covers spelling drift
# systematically.
#
# Each root is only combined with a bounded, explicit suffix list (never an
# open wildcard), so this can't accidentally swallow a real name that merely
# starts with the same letters - e.g. "Karim"/"Kareem"/"Chahat" are real
# Pakistani names, but "Karim" = "kar" + "im", and "im" is not in the suffix
# list below, so it is correctly left alone. Only combinations that are
# actually grammatical Roman Urdu verb forms match.
RECIPIENT_STOP_VERB_ROOTS_RU = ['bhej', 'bhij', 'kar', 'kr', 'chah']

RECIPIENT_STOP_VERB_SUFFIXES_RU = [
    '', 'o', 'do', 'na', 'ne', 'nay', 'ney', 'nei', 'ni', 'niye', 'iye',
    'ein', 'ien', 'enge', 'ta', 'ti', 'te', 'tay', 'wao', 'wa', 'dena',
    'dein', 'den',
]


def _is_ru_verb_form(word: str) -> bool:
    """True if `word` is a known Roman Urdu verb root plus a recognized
    suffix (e.g. "bhejnei", "karnay", "chahiye" all match), covering
    spelling variants that a fixed word list would otherwise miss.
    Deliberately conservative: the suffix set is small and explicit, so a
    real name that happens to share a root's first letters (e.g. "Karim",
    "Chahat") is never matched unless the remainder is an exact known
    grammatical suffix.
    """
    word = word.lower()
    for root in RECIPIENT_STOP_VERB_ROOTS_RU:
        if word.startswith(root) and word[len(root):] in RECIPIENT_STOP_VERB_SUFFIXES_RU:
            return True
    return False
...
def extract_recipient_name(text: str) -> Optional[str]:
    ...
    Uses the categorized RECIPIENT_STOP_* sets (command verbs, intent/desire
    verbs like "want"/"chahiye", prepositions, pronouns/particles, and
    financial/currency words - each covering both English and Roman Urdu)
    plus RECIPIENT_STOP_MULTIWORD_PHRASES for phrases like "bhej do" or
    "want to send" that read more reliably as a unit, so none of these can
    ...
    working_text = text
    multi_word = sorted(RECIPIENT_STOP_MULTIWORD_PHRASES, key=len, reverse=True)
    for phrase in multi_word:
        working_text = re.sub(
            r'\b' + re.escape(phrase) + r'\b', '', working_text, flags=re.IGNORECASE
        )

    # Also exclude any Urdu-script token (they are verbs/particles, not
    # names), and any Roman Urdu verb-form (root+suffix match) that a fixed
    # word list would miss due to spelling variation (e.g. "bhejnei").
    name_words = [
        w for w in working_text.split()
        if w.lower() not in RECIPIENT_STOPWORDS
        and not _is_ru_verb_form(w)
    ...
    # Final safety: reject if the candidate is a known stopword (from any
    # category above) or a banking-domain word that slipped through, even
    # if it somehow survived per-word filtering above (e.g. a candidate
    # that only becomes a single leftover token after stripping).
    _name_blocklist = RECIPIENT_STOPWORDS | {
        'electricity', 'gas', 'water', 'internet', 'bill',
    }
    if candidate.lower() in _name_blocklist or _is_ru_verb_form(candidate):
        return None
    # Reject a candidate composed ENTIRELY of stopword tokens / verb-form
    # matches even when multiple words survive the per-word filter as a
    # set (defense in depth against any new stopword category added above
    # without a matching entry in RECIPIENT_STOPWORDS).
    if all(w.lower() in _name_blocklist or _is_ru_verb_form(w) for w in candidate.split()):
        return None
...
def run_flow_step(user_message: str, ctx: Dict, language: str,
    ...
    # Special-case the very first slot of transfer_money (recipient): also
    # opportunistically pull the amount out of the SAME message, e.g. "send
    # 1200 to Ahmed" or "Ahmed ko 1200 bhejdo" already tells us both the
    # recipient AND the amount in one turn. We still ask for account_number
    # next (it's the next slot in FLOW_SLOT_ORDER), but we don't force a
    # redundant re-ask for the amount later since it's already known.
    if current_flow == 'transfer_money' and target_slot == 'recipient':
        opportunistic_amount = extract_amount(user_message)
        if opportunistic_amount is not None:
            new_ctx['amount'] = opportunistic_amount
