...
SLANG_MAPPING = {
    ...
    'ssgc': 'gas',
    'sngpl': 'gas',
    # Electricity providers / short forms
...
RESPONSES = {
    ...
    'investment_redirect': {
        'en': "Great question! For personalized investment and wealth-growth guidance, I'd recommend chatting with Fin, our dedicated financial advisor. Head on over to the Financial Adviosry Section!",
        ...
        'ru': "Bohat acha sawal! Investment aur wealth growth ki personalized guidance ke liye, hamare dedicated financial advisor Fin se baat karein. Shuru karne ke liye Advisory Section me bubble par tap karein."
    },
    'identity_name': {
...
def extract_embedded_account_number(text: str) -> Optional[str]:
    ...
    account number the user already gave.
    """
    match = re.search(
...
# in `run_flow_step` (see below). Some slot extractors (like
# `validate_account_number`) are deliberately strict because they're also
# used to parse a bare, single-purpose reply (e.g. the user's entire
# message when we're sitting in TRANSFER_AWAIT_ACCOUNT is just "123456").
# That strictness makes them reject a full free-form sentence like "Send
# 5000 to Ali account 123456" where the same value is embedded alongside
# other words. These overrides supply a looser, sentence-aware extractor
# for exactly that merge-pass use case, without touching the strict
# extractor used for the normal single-slot turn (so standalone-reply
# validation behavior is completely unchanged).
MULTI_SLOT_EXTRACTOR_OVERRIDES = {
    ('transfer_money', 'account_number'): extract_embedded_account_number,
}
...
def _merge_multi_slot_extraction(current_flow: str, user_message: str, ctx: Dict,
    ...
    still outstanding. This is what lets "Send 5000 to Ali account
    123456" (or the same details spread across several turns) fill amount
    + recipient + account_number in one pass instead of re-prompting for
    slots the user has already supplied.

    transfer_money gets a dedicated ordering (account_number, then amount,
    then recipient) because the slots are textually entangled: the
    embedded account-number match is stripped out of the working text
    first so its digits can't leak into the amount/recipient extractors,
    and the recipient extractor already strips the amount internally. Any
    other flow (currently just pay_bill) falls back to a straightforward
    per-slot pass since its slots don't have the same entanglement.
    ...
    if current_flow == 'transfer_money':
        ...
        if not merged_ctx.get('account_number'):
            candidate = extract_embedded_account_number(working_text)
            if candidate:
                merged_ctx['account_number'] = candidate
                any_merged = True
                working_text = re.sub(
                    r'\b(?:account|acc|a\/c)\.?\s*(?:number|no\.?|#)?\s*[:\-]?\s*'
                    + re.escape(candidate),
        ...
        if not merged_ctx.get('amount'):
            try:
                candidate = extract_amount(working_text)
            except Exception:
                candidate = None
            if candidate is not None:
                merged_ctx['amount'] = candidate
                any_merged = True

        if not merged_ctx.get('recipient'):
            try:
                candidate = extract_recipient_name_for_transfer_followup(working_text)
            except Exception:
                candidate = None
            if candidate:
                merged_ctx['recipient'] = candidate
                any_merged = True

        return merged_ctx, any_merged

    if current_flow == 'pay_bill':
        # bill_type is a pure keyword match (doesn't consume any text), so
        # it's safe to extract first without touching working_text.
        working_text = user_message
        if not merged_ctx.get('bill_type'):
            try:
                candidate = extract_bill_type(working_text)
            except Exception:
                candidate = None
            if candidate:
                merged_ctx['bill_type'] = candidate
                any_merged = True

        # bill_reference next, and - same reasoning as the transfer_money
        # account-number case above - strip the matched token out of the
        # working text afterwards so its digits can't also get picked up
        # as the (much larger) bill amount by extract_amount's
        # no-currency-marker "take the largest bare number" fallback.
        if not merged_ctx.get('bill_reference'):
            try:
                candidate = extract_bill_reference(working_text)
            except Exception:
                candidate = None
            if candidate:
                merged_ctx['bill_reference'] = candidate
                any_merged = True
                working_text = re.sub(
                    r'\b' + re.escape(candidate) + r'\b', ' ', working_text,
                    flags=re.IGNORECASE,
                )
    ...
    # Generic fallback for any other table-driven flow.
...
def run_flow_step(user_message: str, ctx: Dict, language: str,
    ...
    # deciding what's still needed, so providing all the details at once
    # ("Send 5000 to Ali account 123456") - or filling in a couple of them
    # over several turns - never re-prompts for a slot that's already been
    # supplied. Only runs on a real user turn (not the empty-message
    ...
    any_merged = False
    if not skip_extraction and user_message:
    ...
    if skip_extraction or any_merged:
        ...
        # to 'account_number' - re-running "Ali" through the
        # account-number extractor below would wrongly report an
        # *invalid* account number instead of just re-prompting for one).
        # Either way, re-enter with an empty message so the caller only
        # ever sees a normal "please provide X" prompt for whatever's
        # still missing, never a spurious invalid-value error.
